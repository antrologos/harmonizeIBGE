...
build_education_fieldsMidschool <- function(CensusData){
        
        CensusData <- harmonizeIBGE:::check_prepared_to_harmonize(CensusData)
        metadata   <- harmonizeIBGE:::get_metadata(CensusData)
        
        if(metadata$year > 1991){
                warning(paste("'fieldsMidschool' is not available for the year", metadata$year))
                return(CensusData)
        }
        ...
        CensusData = CensusData[crosswalk, fieldsMidschool := field_code] # this causes loss of the metadata
        gc(); Sys.sleep(.3);gc()
        ...
        CensusData[educationAttainment %in% c(1,2,9, 99), fieldsMidschool := NA]
        CensusData[educationAttainment %in% 3:8 & is.na(fieldsMidschool), fieldsMidschool := 90]
