...
build_identification_idhh_1970 <- function(CensusData){
        ...
        warning("This function requires the data to be never resorted before. The order of the lines must be\nexactly the same as in the original IBGE files.")

        CensusData[ , flag := as.numeric(NA)]

        # (1) A person who is head of a family starts a new household (except if her family is a secondary family)
        CensusData[v025 == 1 & (v006 %in% c(1,2)), flag := 1]
